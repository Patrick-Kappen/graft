...
func mergeNetworkUnits(base, override []NetworkUnitConfig) []NetworkUnitConfig {
	byName := make(map[string]struct{}, len(base))
	result := append([]NetworkUnitConfig(nil), base...)
	for _, n := range base {
...
func mergeVolumeUnits(base, override []VolumeUnitConfig) []VolumeUnitConfig {
	byName := make(map[string]struct{}, len(base))
	result := append([]VolumeUnitConfig(nil), base...)
	for _, v := range base {
...
func mergeSecrets(base, override []SecretConfig) []SecretConfig {
	byName := make(map[string]struct{}, len(base))
	result := append([]SecretConfig(nil), base...)
	for _, s := range base {
