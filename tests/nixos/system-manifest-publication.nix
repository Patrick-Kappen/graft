...
in
{
  ...
  testScript = ''
    ...
    machine.succeed("test $(stat -c '%a:%U:%G:%F' /run/graft/system/worker.sock) = 660:root:graft:socket")
    machine.succeed("systemctl is-active graft-system-worker.socket")
    machine.succeed("systemctl cat graft-system-worker.service | grep -F -- '--target system --effective-uid 0 --manager system'")
    machine.succeed("systemctl cat graft-system-worker.service | grep -F -- '--graft-gid'")
    machine.succeed("systemctl cat graft-system-worker.service | grep -F -- '--producer-name graft'")
    machine.succeed("systemctl show graft-system-worker.service -p Restart -p RestartUSec | grep -F 'Restart=on-failure'")
    machine.succeed("systemctl start graft-system-worker.service")
    machine.succeed("systemctl is-active graft-system-worker.service")
    machine.succeed("systemctl stop graft-system-worker.service")
