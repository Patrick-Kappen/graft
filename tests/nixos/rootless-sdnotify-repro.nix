# Minimal rootless sd_notify readiness reproducer for issue #368.
#
# Independent of Graft activation: a linger user manager + a single Quadlet
# .container unit (Type=notify / NotifyAccess=all / --sdnotify=conmon -d /
# ExitType=cgroup) that must reach `active` on every boot.  Repeats reboots
# and, on any ready-timeout, collects the bounded evidence required by
# issue #368 *before* teardown so the failure mode can be distinguished:
#
#   - no READY=1 sent at all
#   - notification sent but rejected/unattributed by the user manager
#   - MAINPID/cgroup handoff state
#   - service stuck in `activating` versus a terminal protocol failure
#
