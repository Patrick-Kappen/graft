...
pub fn resolve(config: &ContainerConfig) -> Result<ResolvedContainer> {
    ...
    let service = config
        ...
        .and_then(|service| service.restart.clone())
        .map(|restart| ResolvedService {
...
fn resolve_packages(runtime: Option<&Runtime>) -> Vec<String> {
...
mod tests {
    ...
    fn missing_name_returns_error() {
        let result = resolve(&ContainerConfig::default());
