...
There are no built-in presets such as `bare`, `safe`, `agent`, or `pi` that
...
See also [`runtime-architecture.md`](runtime-architecture.md) for the planned
separation between TOML config engine, Quadlet runtime manager, and
