...
| Host-path, sensitive-source, or writable-host mounts | Literal `config.filesystem.volumes` parts are delimiter- and line-safe but have no semantic source/path/mode, existence, or target-overlap policy | Dangerous | Current narrow passthrough; policy pending [#142] and [#163] |
