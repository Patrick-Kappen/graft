...
| `config.filesystem.tmpfs` | Optional ordered list of unique absolute container paths; empty is omitted; control characters and `:` fail | `filesystem.tmpfs` | Passed through mechanically | One ordered writable `Tmpfs=<path>` line per entry | Both | Current path-only phase of [#164] |
...
| `config.filesystem.readOnlyTmpfs` and `mounts` | Field-specific resolver error | Broader mount and root-filesystem policy remain in [#142], [#163], and [#164] |
