...
| `deploy.target` | Required `system` or `user`; omission fails | Effective `deploy.target` | Selects NixOS or Home Manager output | Selects system or user manager | Both | Current |
...
| `config.filesystem.binds[].target` | Absolute normalised protected and collision-checked target | Preserved | Passed to renderer | Target component | Both | Current |
| `config.filesystem.binds[].readOnly` | Optional boolean; defaults to `true` | Concrete boolean | Passed to renderer | Explicit `ro,bind` or `rw,bind` | Both | Current |
| `config.filesystem.volumes` | Optional ordered typed managed volumes; empty is omitted | Concrete `filesystem.volumes` | Validated with all mount targets | Ordered `Volume=` lines | Both | Current through [#164] |
| `config.filesystem.volumes[].name` | Optional literal named-volume resource; absence requests anonymous storage | Preserved optional name | Name grammar validated | Named source component when present | Both | Current |
| `config.filesystem.volumes[].target` | Absolute normalised protected and collision-checked target | Preserved | Passed to renderer | Target component | Both | Current |
| `config.filesystem.volumes[].readOnly` | Optional boolean; defaults to `false`; `true` requires a name | Concrete boolean | Passed to renderer | Named volumes use explicit `ro` or `rw`; anonymous form is writable | Both | Current |
...
| `config.security.privileged`, `seccompProfile`, `securityLabelDisable`, `securityLabelFileType`, `securityLabelLevel`, `securityLabelNested`, `securityLabelType`, `securityOpt`, and `userns` | Field-specific resolver error | These fields retain their documented deferred or forbidden boundaries |
...
| Capability additions | Ordered canonical additions are accepted after drop-all | Dangerous | Current through [#163]; rootless additions remain bounded by the container user namespace and runtime authorization |
...
| Host-path, sensitive-source, or writable-host mounts | Typed binds default read-only; writable access requires `readOnly = false`; protected lexical sources and colliding targets fail, while source type and symlink traversal remain unattested | Dangerous | Current through [#164]; raw options, propagation, and sensitive virtual sources remain unavailable |
| Anonymous managed volumes | Typed source-less entries request writable anonymous storage; `readOnly = true` fails | First-class | Current through [#164]; lifecycle follows generated container cleanup behavior |
| Named managed volumes | Typed literal names select persistent resources with explicit `ro` or `rw` access | Dangerous | Current through [#164]; existing state and same-scope sharing remain explicit host/runtime boundaries |
