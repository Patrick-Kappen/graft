...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Prevent NixOS from auto-restarting containers that opt out via
    # [config.service] restartIfChanged = false in their TOML.
    systemd.services = lib.listToAttrs (
      lib.concatMap (
        entry:
        if (entry.effectiveConfig.service.restartIfChanged or true) == false then
          [ { name = entry.name; value = { restartIfChanged = false; }; } ]
        else
          [ ]
      ) evaluated.activeEntries
    );
