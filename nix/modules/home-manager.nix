    ...
    assertions = [
      {
        ...
        message = "programs.graft: active TOML config names must be unique.";
      }
    ]
    ++ map (entry: {
      assertion = !entry.invalidDeployTarget;
      message = "programs.graft: invalid deploy target in ${toString entry.configFile}: ${entry.configDeployTarget}";
    }) evaluated.entries
    ++ map (entry: {
      assertion = !entry.invalidRuntimeMode;
      message = "programs.graft: unsupported runtime mode in ${toString entry.configFile}; expected rootfs-store";
    }) evaluated.activeEntries
    ++ map (entry: {
      assertion = entry.invalidPackageNames == [ ];
      message = "programs.graft: invalid package name in ${toString entry.configFile} config.runtime.packages: ${builtins.concatStringsSep ", " entry.invalidPackageNames}";
    }) evaluated.activeEntries
    ++ map (entry: {
      assertion = entry.unknownPackageNames == [ ];
      message = "programs.graft: unknown package in ${toString entry.configFile} config.runtime.packages: ${builtins.concatStringsSep ", " entry.unknownPackageNames}";
    }) evaluated.activeEntries;

    home.packages = [ cfg.package ];

    # After home-manager writes the Quadlet unit files, reload the systemd user
    # daemon so Podman Quadlet picks up new or changed units automatically.
    # The || true prevents failure when no user session is active yet (e.g. first install).
    home.activation.graftReload = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
